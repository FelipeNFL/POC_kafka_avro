...
def post_data_additional():

  print_title('Schema v1 - Campos adicionais não definidos no schema')
  ...
  data = {'indicators': 'gross_revenue', 'notes': 'serasa', 'tag_adicional': 'Nelson'}

  produce(schema, data)


def post_data_missing():

  print_title('Schema v1 - Campos definidos no schema não enviados')
...
#TODO testar trocar compatibilidade quando já há schemas cadastrados em outra compatibilidade
...
if __name__ == "__main__":
  ...
  # global_compatibility = {"compatibility": "NONE"}
  # config_endpoint = '{url}/config'.format(url=schema_registry_url)

  # res = requests.put(url=config_endpoint, json=compatibility)

  # print(res.json())

  options = [post_data_additional,
             post_data_missing]
